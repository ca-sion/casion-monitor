    ...
    <div class="bg-white rounded-lg shadow-md p-6">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-900 mb-2">
                Ajouter un protocole de récupération
            </h1>
            @if ($injury)
                <div class="text-sm text-gray-600">
                    <p><strong>Lié à la blessure :</strong> {{ $injury->type }} - {{ $injury->location }}</p>
            ...
            @else
                <p class="text-sm text-gray-600">Ce protocole n'est pas lié à une blessure spécifique.</p>
            @endif
        </div>
        ...
        <div class="border-t border-gray-200 pt-6">
            <form wire:submit="save">
                ...
                <div class="mt-6 flex items-center justify-between">
                    ...
                    @else
                        <a href="{{ route('athletes.dashboard', ['hash' => $athlete->hash]) }}" 
                           class="inline-flex items-center px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"/>
                            </svg>
