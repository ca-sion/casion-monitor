...
class MetricStatisticsService
{
     ...
     */
    protected function getAthleteWeeklyMetricsSummary(Athlete $athlete, Carbon $weekStartDate, ?Collection $allMetrics = null): array
    {
        $weekEndDate = $weekStartDate->copy()->endOfWeek(Carbon::SUNDAY);
     ...
     * @param  Collection<string, array>  $athleteWeeklyMetrics  Les données hebdomadaires pré-calculées pour l'athlète.
     * @return array Les données formatées pour le tableau de bord.
     */
    public function getDashboardWeeklyMetricData(Athlete $athlete, CalculatedMetric $metricKey, string $period, Collection $athleteWeeklyMetrics): array
    {
        $now = Carbon::now();
        ...
        $lastValue = $allWeeklyData->sortByDesc('date')->first()->value ?? null;
        $metricData['last_value'] = $lastValue;
        $metricData['formatted_last_value'] = is_numeric($lastValue) ? number_format($lastValue, 1) : 'N/A';
